...
void CosocoCallbacks::beginInstance(InstanceType type) {
    ...
    for (int core = 0; core < nbcores; core++)
        problems[core] = type == CSP ? new Problem("") : new OptimizationProblem("");
...
void CosocoCallbacks::buildConstraintExtension(string id, vector<XVariable *> list, vector<vector<int>> &origTuples,
                                               bool support,
                                               bool hasStar) {
    if (hasStar && !support)
...
void CosocoCallbacks::buildConstraintExtension2(const string &id, vec<Variable *> &scope,
                                                const vector<vector<int>> &origTuples,
...
void CosocoCallbacks::buildConstraintExtension(string id, XVariable *variable, vector<int> &tuples, bool support,
                                               bool hasStar) {
    if (hasStar)
        throw runtime_error("Extension constraint with star is not yet supported");

    for (int core = 0; core < nbcores; core++)
        FactoryConstraints::createConstraintUnary(problems[core], id, problems[core]->mapping[variable->id],
                                                  vector2vec(tuples),
                                                  support);
}
...
void CosocoCallbacks::buildConstraintIntension(string id, Tree *tree) {
    ...
    if (tree->root->type == OEQ && tree->root->parameters[0]->type == ODIST &&
        tree->root->parameters[0]->parameters[0]->type == OVAR &&
        tree->root->parameters[0]->parameters[1]->type == OVAR &&
        tree->root->parameters[1]->type == OVAR) {
        for (int core = 0; core < nbcores; core++) {
            FactoryConstraints::createConstraintDistXYeqZ(problems[core], id,
                                                          problems[core]->mapping[tree->listOfVariables[1]],
    ...
    if (tree->root->type == OEQ && tree->root->parameters[0]->type == OVAR && tree->root->parameters[1]->type == OSUB &&
        tree->root->parameters[1]->parameters[0]->type == OVAR &&
        tree->root->parameters[1]->parameters[1]->type == OVAR &&
        scope.size() == 3) {
        for (int core = 0; core < nbcores; core++)
...
*/
    ...
    if (startToParseObjective == false && intension2extensionLimit &&
        nbTuples < intension2extensionLimit) {   // Create extension
        nbIntension2Extention++;
...
void
CosocoCallbacks::buildConstraintPrimitive(string id, OrderType op, XVariable *x, int k, XVariable *y) {   // x + k op y
...
void
CosocoCallbacks::buildConstraintPrimitive(string id, OrderType op, XVariable *xx, int k) {   // x op k op is <= or >=
    for (int core = 0; core < nbcores; core++) {
...
void
CosocoCallbacks::buildConstraintOrdered(string id, vector<XVariable *> &list, vector<int> &lengths, OrderType order) {
    for (int core = 0; core < nbcores; core++)
        FactoryConstraints::createConstraintOrdered(problems[core], id, toMyVariables(list, core), lengths, order);
}
...
void
CosocoCallbacks::buildConstraintSum(string id, vector<XVariable *> &list, vector<int> &origcoeffs, XCondition &xc) {
    for (int core = 0; core < nbcores; core++) {
        vars.clear();
        toMyVariables(list, vars, core);
...
void
CosocoCallbacks::buildConstraintSum(string id, vector<XVariable *> &list, vector<XVariable *> &coeffs, XCondition &xc) {
    bool scalar = true;
    ...
    for (unsigned int i = 0; i < list.size(); i++)
        trees.push_back(new Tree("mul(" + list[i]->id + "," + coeffs[i]->id + ")"));
...
void CosocoCallbacks::buildConstraintCount(string id, vector<XVariable *> &list, vector<XVariable *> &values,
                                           XCondition &xc) {
    if (values.size() == 1) {
        vector<Tree *> trees;
        for (auto &xv: list) {
...
void
CosocoCallbacks::buildConstraintCount(string id, vector<Tree *> &trees, vector<XVariable *> &values, XCondition &xc) {
    if (values.size() == 1) {
        vector<Tree *> newtrees;
        for (auto &tree: trees) {
...
void CosocoCallbacks::buildConstraintCardinality(string id, vector<XVariable *> &list, vector<int> values,
    ...
    for (int core = 0; core < nbcores; core++) {
        ...
        FactoryConstraints::createConstraintCardinality(problems[core], id, toMyVariables(list, core),
                                                        vector2vec(values),
                                                        occurs);
    }
}
    ...
    if (xc.operandType == VARIABLE) {
        if (xc.op == EQ) {
            ...
            for (int core = 0; core < nbcores; core++) {
                vars.clear();
                for (auto &auxiliaryVariable: auxiliaryVariables) vars.push(problems[core]->mapping[auxiliaryVariable]);
                FactoryConstraints::createConstraintMaximumVariableEQ(problems[core], id, vars,
                                                                      problems[core]->mapping[xc.var]);
            }
            return;
        }
    ...
    if (xc.operandType == VARIABLE) {
        if (xc.op == EQ) {
            ...
            for (int core = 0; core < nbcores; core++) {
                vars.clear();
                for (auto &auxiliaryVariable: auxiliaryVariables) vars.push(problems[core]->mapping[auxiliaryVariable]);
                FactoryConstraints::createConstraintMinimumVariableEQ(problems[core], id, vars,
                                                                      problems[core]->mapping[xc.var]);
            }
            return;
        }
...
void CosocoCallbacks::buildConstraintMaximum(string id, vector<XVariable *> &list, XCondition &xc) {
    ...
    if (xc.operandType == INTEGER) {
        if (xc.op == LE || xc.op == LT) {
            for (int core = 0; core < nbcores; core++) {
                toMyVariables(list, vars, core);
                FactoryConstraints::createConstraintMaximumLE(problems[core], id, vars,
                                                              xc.op == LE ? xc.val : xc.val - 1);
            }
            return;
        }
        if (xc.op == GE || xc.op == GT) {
            for (int core = 0; core < nbcores; core++) {
                toMyVariables(list, vars, core);
                FactoryConstraints::createConstraintMaximumGE(problems[core], id, vars,
                                                              xc.op == GE ? xc.val : xc.val + 1);
            }
            return;
        }
...
void CosocoCallbacks::buildConstraintMinimum(string id, vector<XVariable *> &list, XCondition &xc) {
    ...
    if (xc.operandType == INTEGER) {
        if (xc.op == LE || xc.op == LT) {
            for (int core = 0; core < nbcores; core++) {
                toMyVariables(list, vars, core);
                FactoryConstraints::createConstraintMinimumLE(problems[core], id, vars,
                                                              xc.op == LE ? xc.val : xc.val - 1);
            }
            return;
        }
        if (xc.op == GE || xc.op == GT) {
            for (int core = 0; core < nbcores; core++) {
                toMyVariables(list, vars, core);
                FactoryConstraints::createConstraintMinimumGE(problems[core], id, vars,
                                                              xc.op == GE ? xc.val : xc.val + 1);
            }
            return;
        }
...
void CosocoCallbacks::buildConstraintElement(string id, vector<XVariable *> &list, int startIndex, XVariable *index,
    ...
    for (int core = 0; core < nbcores; core++) {
        ...
        FactoryConstraints::createConstraintElementConstant(problems[core], id, toMyVariables(list, core), idx,
                                                            startIndex,
                                                            value);
    }
}
...
void CosocoCallbacks::buildConstraintElement(string id, vector<XVariable *> &list, int startIndex, XVariable *index,
    ...
    for (int core = 0; core < nbcores; core++) {
        ...
        assert(val != nullptr);
        FactoryConstraints::createConstraintElementVariable(problems[core], id, toMyVariables(list, core), idx,
                                                            startIndex, val);
    }
}
...
void
CosocoCallbacks::buildConstraintElement(string id, vector<int> &list, int startIndex, XVariable *index, RankType rank,
                                        XVariable *value) {
    for (int core = 0; core < nbcores; core++) {
        vec<Variable *> tmp;
        vec<vec<int>> tuples;
...
void
CosocoCallbacks::buildConstraintElement(string id, vector<vector<int>> &matrix, int startRowIndex, XVariable *rowIndex,
...
void CosocoCallbacks::buildConstraintChannel(string id, vector<XVariable *> &list1, int startIndex1,
...
void CosocoCallbacks::buildConstraintNoOverlap(string id, vector<XVariable *> &origins, vector<int> &lengths,
                                               bool zeroIgnored) {
    if (!zeroIgnored)
    ...
    for (int core = 0; core < nbcores; core++) {
        ...
        for (int i = 0; i < vars.size(); i++)
            for (int j = i + 1; j < vars.size(); j++)
                FactoryConstraints::createConstraintNoOverlap(problems[core], id, vars[i], vars[j], lengths[i],
                                                              lengths[j]);
    }
}
...
void
CosocoCallbacks::buildConstraintNoOverlap(string id, vector<vector<XVariable *>> &origins, vector<vector<int>> &lengths,
                                          bool zeroIgnored) {
    if (!zeroIgnored)
        throw runtime_error("K dim Nooverlap with zeroIgnored not yet supported");
    ...
    for (unsigned int i = 0; i < origins.size(); i++) {
        for (unsigned int j = i + 1; j < origins.size(); j++) {
            string le1 =
                    "le(add(" + origins[i][0]->id + "," + std::to_string(lengths[i][0]) + ")," + origins[j][0]->id +
                    ")";
            string le2 =
                    "le(add(" + origins[j][0]->id + "," + std::to_string(lengths[j][0]) + ")," + origins[i][0]->id +
                    ")";

            string le3 =
                    "le(add(" + origins[i][1]->id + "," + std::to_string(lengths[i][1]) + ")," + origins[j][1]->id +
                    ")";
            string le4 =
                    "le(add(" + origins[j][1]->id + "," + std::to_string(lengths[j][1]) + ")," + origins[i][1]->id +
                    ")";
    ...
    for (int core = 0; core < nbcores; core++)
        for (unsigned int i = 0; i < list.size(); i++) {
            ...
            if (values[i] == STAR)
                continue;
            value.push(values[i]);
            FactoryConstraints::createConstraintUnary(problems[core], id, problems[core]->mapping[list[i]->id], value,
                                                      true);
        }
}
...
void CosocoCallbacks::buildConstraintKnapsack(string id, vector<XVariable *> &list, vector<int> &weights,
...
void
CosocoCallbacks::buildConstraintFlow(string id, vector<XVariable *> &list, vector<int> &balance, vector<int> &weights,
...
void
CosocoCallbacks::buildObjectiveMinimize(ExpressionObjective type, vec<Variable *> &variables, vector<int> &origcoeffs,
                                        int core) {
    auto *po = static_cast<OptimizationProblem *>(problems[core]);
    po->type = OptimisationType::Minimize;
    ...
    switch (type) {
        ...
        case SUM_O: {
            ...
            FactoryConstraints::createConstraintSum(problems[core], "objective", variables, vector2vec(origcoeffs),
                                                    INT_MAX, LE);
            auto *oc = dynamic_cast<ObjectiveConstraint *>(problems[core]->constraints.last());
            po->addObjectiveLB(oc, true);
            break;
...
void
CosocoCallbacks::buildObjectiveMinimize(ExpressionObjective type, vector<XVariable *> &list, vector<int> &origcoeffs) {
    for (int core = 0; core < nbcores; core++) {
        toMyVariables(list, vars, core);
...
void
CosocoCallbacks::buildObjectiveMaximize(ExpressionObjective type, vec<Variable *> &variables, vector<int> &origcoeffs,
                                        int core) {
    auto *po = static_cast<OptimizationProblem *>(problems[core]);
    po->type = OptimisationType::Maximize;
...
void
CosocoCallbacks::buildObjectiveMaximize(ExpressionObjective type, vector<XVariable *> &list, vector<int> &origcoeffs) {
    for (int core = 0; core < nbcores; core++) {
        toMyVariables(list, vars, core);
...
void
CosocoCallbacks::createAuxiliaryVariablesAndExpressions(vector<Tree *> &trees, vector<string> &auxiliaryVariables) {
    set<int> values;
