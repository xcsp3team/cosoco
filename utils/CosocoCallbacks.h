...
class CosocoCallbacks : public XCSP3CoreCallbacks {
   ...
   public:
    ...
    void endVariables() override { nbInitialsVariables = problems[0]->nbVariables(); }
    ...
    void beginGroup(string name) override { initGroups(); }


    void endGroup() override { insideGroup = false; }


    void beginSlide(string id, bool circular) override { initGroups(); }
    ...
    void beginBlock(string classes) override { initGroups(); }
    ...
    void beginObjectives() override { startToParseObjective = true; }
    ...
    void buildConstraintIntension(string id, Tree *tree) override {
        ...
        if(tree->root->type == OEQ && tree->root->parameters[0]->type == ODIST &&
           ...
           tree->root->parameters[1]->type == OVAR) {
            for(int core = 0; core < nbcores; core++) {
                FactoryConstraints::createConstraintDistXYeqZ(
                    problems[core], id, problems[core]->mapping[tree->listOfVariables[1]],
                    problems[core]->mapping[tree->listOfVariables[2]], problems[core]->mapping[tree->listOfVariables[0]]);
            }
            return;
        }
...
*/
        ...
        if(startToParseObjective == false && insideGroup && i2eNumber >= 0 && arguments() != nullptr &&
           (*arguments())[nbIntension].size() == (*arguments())[i2eNumber].size()) {
            ...
            if(sameArguments == true) {   // And same domains -> create extension and leave
                Extension *c = dynamic_cast<Extension *>(problems[0]->constraints.last());
                assert(c != nullptr);
    ...
    void buildConstraintPrimitive(string id, OrderType op, XVariable *x, int k, XVariable *y) override {   // x + k op y
    ...
    void buildConstraintPrimitive(string id, XVariable *xx, bool in, int min, int max) override {   // x in/notin [min,max]
        for(int core = 0; core < nbcores; core++) {
            Variable *x = problems[core]->mapping[xx->id];
            vec<int>  values;
            for(int idv : x->domain) {
    ...
    virtual void buildConstraintCircuit(string id, vector<XVariable *> &list, int startIndex) override {
        if(startIndex != 0)
            throw("Circuit with startIndex != 0 is not yet supported");
    ...
    void buildConstraintElement(string id, vector<XVariable *> &list, int startIndex, XVariable *index, RankType rank,
        ...
        for(int core = 0; core < nbcores; core++) {
            ...
            assert(val != nullptr);
            FactoryConstraints::createConstraintElementVariable(problems[core], id, toMyVariables(list, core), idx, startIndex,
                                                                val);
        }
    }
    ...
    void buildConstraintKnapsack(string id, vector<XVariable *> &list, vector<int> &weights, vector<int> &profits, int limit,
                                 XCondition &xc) override {
        for(int core = 0; core < nbcores; core++) {
            toMyVariables(list, core);
    ...
    void buildConstraintKnapsack(string id, vector<XVariable *> &list, vector<int> &weights, vector<int> &profits,
                                 XVariable *limit, XCondition &xc) override {
        weights.push_back(-1);
        for(int core = 0; core < nbcores; core++) {
            toMyVariables(list, core);
    ...
    void buildObjectiveMinimize(ExpressionObjective type, vec<Variable *> &variables, vector<int> &origcoeffs, int core) {
        ...
        switch(type) {
            ...
            case SUM_O: {
                ...
                FactoryConstraints::createConstraintSum(problems[core], "objective", variables, vector2vec(origcoeffs), INT_MAX,
                                                        LE);
                ObjectiveConstraint *oc = dynamic_cast<ObjectiveConstraint *>(problems[core]->constraints.last());
                po->addObjectiveLB(oc, true);
                break;
    ...
    void buildObjectiveMaximize(ExpressionObjective type, vec<Variable *> &variables, vector<int> &origcoeffs, int core) {
        ...
        switch(type) {
            case EXPRESSION_O:
                break;   // useless, want to discard warning
            case SUM_O: {
                FactoryConstraints::createConstraintSum(problems[core], "objective", variables, vector2vec(origcoeffs), INT_MIN,
                                                        GE);
                break;
            }
            case PRODUCT_O:
