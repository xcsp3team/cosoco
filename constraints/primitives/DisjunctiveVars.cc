...
bool DisjunctiveVars::filter(Variable *xx) {
    ...
    if(!b2)   // we enforce the first part
        return solver
                   ->delValuesLowerOrEqualThan(x2)
               // return dx2.removeValuesLT(min1) && dx1.removeValuesGT(max1) &&
               // dw1.removeValuesGT(dx2.lastValue() - dx1.firstValue());
               if(!b1)   // we enforce the second part
               return dx1.removeValuesLT(min2) &&
               dx2.removeValuesGT(max2) && dw2.removeValuesGT(dx1.lastValue() - dx2.firstValue());
